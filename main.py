...
time_units = {"1": "days", "2": "weeks", "3": "months", "4": "years"}
...
amount_of_time = input(f"How many {time_units[time_unit]} have you been measuring your tree's circumference? ")
...
for i in range(1, int(amount_of_time) + 1):
  tree_circ = input(f"What was your tree's circumference in {time_units[time_unit]} {i}? ")
...
if time_units[time_unit] is "years":
  oxygen_output_kg = float(tree_circ_total) * 0.4
print(f"Your yearly oxygen output for the tree is approximately: ~{oxygen_output_kg}kg.")
